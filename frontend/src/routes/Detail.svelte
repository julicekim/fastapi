<script>
  ...
  function post_answer(event) {
    ...
    let url = "/api/answer/create/" + question.id;
    let params = {
      content: content
    }
...
<div class="container my-3">
      ...
      <div class="d-flex justify-content-end">
        {#if question.modify_date }
          <div class="badge bg-light text-dark p-2 text-start mx-3"> 
            ...
            <div>{moment(question.modify_date_date).format("YYYY년MM월DD일 hh:mm a")}</div>
          </div>
        {/if}
        <div class="badge bg-light text-dark p-2 text-start">
          <div class="mb-2">{question.user ? question.user.username : ""}</div>
          <div>{moment(question.create_date).format("YYYY년MM월DD일 hh:mm a")}</div>
        </div>
      </div>
      <div class="my-3"> 
  ...
  <h6 class="border-bottom my-3 py-2">{question.answers.length} 개의 답변이 있습니다.</h6>
  ...
  <form method="post" class="my-3">
    <div class="mb-3">
      <textarea rows="10" bind:value={content} class="form-control" disabled={$is_login ? '' : 'disabled'}></textarea>
    </div>
